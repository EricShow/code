...
class CoronaryDirectionNetwork(nn.Module):
    ...
    @torch.jit.ignore
    def forward(self, img0, img1, img2, joint_label, direction_label):
        ...
        outs = [d_predict,c_predict]
        #print(outs.shape)
        head_outs = self.head(outs)
        loss = self.head.loss(head_outs, [direction_label, joint_label])
        return loss
