...
def find_nodules(bboxInfo, Z_THRESHOLD=3, SAME_BOX_THRESHOLD=0.7, SCORE_THRESHOLD=0.6):
        ...
        if len(lst) > 1:
            for j1 in range(len(lst.index)):
                for j2 in range(j1 + 1, len(lst.index)):
                    if calcDICE(
                        get_bounding_box_nparray(lst.iloc[j1]), get_bounding_box_nparray(lst.iloc[j2])
                    ) > SAME_BOX_THRESHOLD:
    ...
    return bboxInfo[['instanceNumber', 'xmin', 'ymin', 'xmax', 'ymax', 'name', 'prob', 'nodule']], nodules
